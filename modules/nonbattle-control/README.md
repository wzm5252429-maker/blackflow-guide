...
连接模块根据 `Arknights.exe` 进程和 Unity 游戏窗口类型确认目标，不依赖标题必须完全一致。最小化、隐藏和不存在会分别提示；不会把浏览器中提到游戏的网页当作游戏窗口。点击「识别预览」前请手动恢复游戏窗口；只有用户点击「自动路线决策」启动的控制会话才会尝试恢复最小化的游戏窗口。

更新本机模块后，需要关闭旧的接管器命令行窗口，再运行 `tools/Start-BlackflowLive.cmd`，从新打开的网页重新连接。已运行的 Python 服务不会自动加载刚更新的代码。先使用「识别预览」检查画面，确认后再自行启动接管。
