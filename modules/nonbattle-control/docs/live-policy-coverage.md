...
连续流程复核进一步发现训练与界面动作的语义差距：预览目前使用 `event`，确认借用 `recruit_reserve`；原训练的后者只表示真实招募券对应的预备干员，并非所有零费普通干员。008 原网络给梓兰预览的 logit 为 -17.610834，地灵确认 -35.918171，确认概率约 1.12e-8。不能仅把所有免费预览改成 reserve 来改善分数；也不能把录像中实际“地灵→深海色”的后继画面当作网络选择梓兰后的结果。需要进一步建立匹配真实招募阶段的策略接口或训练证据，现有 UI 合法性检查不等于网络具备完整招募决策能力。
