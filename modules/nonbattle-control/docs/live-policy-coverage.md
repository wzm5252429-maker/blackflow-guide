...
普通招募新增同帧 `metadata.recruitment_state`：保留全部已定位可见卡片，包括未知身份或费用；明确区分高亮、左侧身份关联、可预览动作及已验证确认动作。显示希望与已验证可用希望分别记录；不把未选卡页面的显示数自动当作余额，不生成正式阵容，不宣称已扫描屏幕外全部候选。原冻结 encoder 尚不消费这个新增合同，当前权重与动作评分不变；这是后续招募意图模型的真实观测输入准备，不能当成已经解决连续招募。

`recruitment_intent.py` 提供未来独立招募头的纯函数绑定接口，尚未接入当前运行时。每个意图引用当前帧、候选索引、精确身份、观测语义摘要和模型来源；inspect 只能查看，不能转为招募承诺。明确 recruit 意图可在目标未选中时绑定真实预览，已选中时须重新验证合同、唯一关联、费用、余额与启用按钮。普通确认使用专用 `confirm_normal_recruitment` 安全合同，旧菜单词表没有新增该操作，因此旧冻结网络不会借用它或借用 reserve 分数。现有检测器尚未证明普通/预备/临时的区分，不能仅以检测器名称填入合同；真实招募记录缺少该证据时绑定器会等待。

绑定器没有点击接口或跨帧持久计划，上一帧意图不会继续执行；仍需未来模型在新帧重新决策并由引擎执行原有双帧、过期、窗口、租约检查。模型来源字符串本身不是已训练/已验收的证明，未来加载器还必须验证独立权重与特征schema。当前正例中的 normal 合同明确为合成测试，不把真实8秒截图的旧reserve动作偷换成已识别普通合同。

独立招募输入准备位于 `recruitment_features.py`：436 个目录身份、职业、客户端星级、观测费用与已知标记、合同类型、选中/动作关联及当前预算组成独立 schema，保留所有已定位卡片，包括身份未知项。目录的 FREE 标记不产生实际费用，候选不进入正式阵容；未扫描阵容与明确观察到的空阵容不同。该编码器不修改原 FeatureEncoder，不生成或加载新权重，不输出排名，也尚未接入运行时。特征可区分身份并不代表已经学习了偏好；下一步仍需真实来源训练、留出验证和实机确认。

未知名字的纯查看采用独立 `recruitment_card_inspect/card_inspect` 合同：仍是无消费的 event 预览，必须有同帧招募页脚、完整卡框和卡内图标，不填写 operator_id，也不冒用原 reserve 确认。该实际候选可进入现有固定菜单网络；新增输入编码和意图绑定器也只允许未知 offer 的 inspect，recruit 仍需真实身份和合同。候选集合扩大后，7.6/7.8/8.0/10.4 四张原图的当前模型均选出查看详情；此变化来自真实可查看候选的加入，未删除既有合法动作或修改权重。没有网络所选卡的实际后继画面，不能宣称完成招募。
