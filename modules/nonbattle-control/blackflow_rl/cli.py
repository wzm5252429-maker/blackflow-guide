...
from .agents import EconomyEvaluator,RelicSearchEnvironment,RelicSearchObjective
from .policy_constraints import DEFAULT_POLICY_CONSTRAINTS
from .client_data import DEFAULT_CLIENT_DATA, validate_client_data
from .evidence import audit_evidence
from .mapgen import (
    MapGenerator,
    MapGeneratorConfig,
    load_map_generation_profile,
    load_node_reward_profile,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    simulate.add_argument(
        "--policy", choices=("random", "heuristic", "economy", "beam", "mcts"), default="economy"
    )
    simulate.add_argument("--simulations", type=int, default=32)
    simulate.add_argument("--max-steps", type=int, default=2000)
    simulate.add_argument(
    ...
    evaluate.add_argument("--policies", nargs="+", choices=("random", "heuristic", "economy", "beam", "mcts"), default=("random", "heuristic", "mcts"))
    evaluate.add_argument(
        "--profile", choices=("evidence", "synthetic"), default="evidence"
    )

    for p in (sample, simulate, plan, train, evaluate):
        p.add_argument("--map-generation-profile", type=Path)
        p.add_argument("--node-reward-profile", type=Path)

    for p in (simulate, evaluate):
        p.add_argument("--policy-config", type=Path, help="EconomyPolicyConfig JSON; preferences do not change simulator rules")
        p.add_argument("--route-config", type=Path, help="RouteSearchConfig JSON for the beam policy")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        map_generation_profile = load_map_generation_profile(
            getattr(args, "map_generation_profile", None)
        )
        node_reward_profile = load_node_reward_profile(
            getattr(args, "node_reward_profile", None)
        )
        ...
        generator = MapGenerator(
            config=MapGeneratorConfig(
                ...
                allow_synthetic_floor6_contents=synthetic,
                reveal_all_floor6=getattr(args, "assume_floor6_full_reveal", False),
            ),
            map_generation_profile=map_generation_profile,
            node_reward_profile=node_reward_profile,
        )
        simulator = BlackflowSimulator(map_generator=generator)
        policy_config = route_config = None
        if args.command in ("simulate", "evaluate"):
            from .agents import EconomyPolicyConfig
            policy_config = EconomyPolicyConfig(**json.loads(args.policy_config.read_text(encoding="utf-8"))) if args.policy_config else EconomyPolicyConfig()
            if args.command == "simulate" and args.policy == "beam" or args.command == "evaluate" and "beam" in args.policies:
                from .route_planner import RouteSearchConfig
                route_config = RouteSearchConfig()
        if getattr(args, "route_config", None):
            from .route_planner import RouteSearchConfig
            if args.command == "simulate" and args.policy != "beam" or args.command == "evaluate" and "beam" not in args.policies:
                raise ValueError("--route-config requires the beam policy")
            route_config = RouteSearchConfig(**json.loads(args.route_config.read_text(encoding="utf-8")))
        ...
        if args.command == "simulate":
            ...
            payload["map_generator_config"] = asdict(generator.config)
            payload["environment_sha256"] = simulator.environment_sha256
            payload["economy_config"] = asdict(simulator.economy.config)
            payload["policy_constraints"] = asdict(DEFAULT_POLICY_CONSTRAINTS)
            payload["policy_constraints_sha256"] = DEFAULT_POLICY_CONSTRAINTS.sha256
            payload["policy_config"] = asdict(policy_config) if policy_config else None
            payload["route_config"] = asdict(route_config) if route_config else None
        ...
        if args.command == "plan":
            ...
            if args.checkpoint:
                ...
                objective=RelicSearchObjective()
                mcts = PUCTMCTS(
                    RelicSearchEnvironment(simulator,objective),
                    EconomyEvaluator(simulator),
                    MCTSConfig(
                        num_simulations=args.simulations,
                        gamma=objective.discount,
                        reward_scale=objective.value_scale,
                        temperature=0.0,
                        seed=args.seed,
                    ),
                )
                ...
                _json(
                    {
                        ...
                        "map_generator_config": asdict(generator.config),
                        "root_value": result.root_value,
                        "policy_constraints": asdict(DEFAULT_POLICY_CONSTRAINTS),
                        "policy_constraints_sha256": DEFAULT_POLICY_CONSTRAINTS.sha256,
                ...
                _json(
                    {
                        ...
                        "policy_constraints_sha256": DEFAULT_POLICY_CONSTRAINTS.sha256,
                        "results": result,
                        "policy_config": asdict(policy_config) if policy_config else None,
                        "route_config": asdict(route_config) if route_config else None,
                    }
                )
            )
            return 0
