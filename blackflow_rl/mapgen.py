...
class NodeRewardProfile:
    """Optional synthetic node payload overrides loaded from evidence/collected rules."""

    incident_pools: Mapping[int, tuple[str, ...]] = field(default_factory=dict)
    node_overrides: Mapping[NodeType, RewardOverride] = field(default_factory=dict)
    incident_overrides: Mapping[str, tuple[EventOption, ...]] = field(
        default_factory=dict
    )
    source_digest: str = "default"


def _floor_key_to_int(value: Any) -> int | None:
    ...
    if isinstance(value, str):
        ...
        if not value:
            return None
        if value in _FLOOR_ROMANS:
            return _FLOOR_ROMANS[value]
        if value.isdigit():
            parsed = int(value)
            if 1 <= parsed <= 6:
                return parsed
...
def _to_range(value: Any) -> tuple[int, int]:
    if isinstance(value, (list, tuple)) and len(value) == 2:
        ...
        return (low, high)
    raise ValueError(f"expected a 2-number range, got {value!r}")


def _normalise_category_ranges(
    raw: Mapping[Any, Any]
) -> dict[int, tuple[int, int]]:
    result: dict[int, tuple[int, int]] = {}
    for name, value in raw.items():
        key = str(name)
        if key not in {"mystery", "ferocity"}:
            raise ValueError(f"invalid content category: {key!r}")
        result[key] = _to_range(value)
    if set(result) != {"mystery", "ferocity"}:
        raise ValueError(f"missing category range keys in {raw!r}")
    return result


def load_map_generation_profile(
    path: str | Path | None,
) -> Mapping[int, Mapping[str, tuple[int, int]]] | None:
    """Load optional synthetic map-generation parameter overrides.

    Supported keys are intentionally small and backward-compatible:
    - {"content_total_ranges": {"1": {"mystery":[1,6], "ferocity":[2,6]}, ...}}
    - {"map_generation": {"content_total_ranges": ...}}
    - {"1": {"mystery":[...], "ferocity":[...]} ...}
    """
    if path is None:
        return None
    ...
    if not isinstance(raw, Mapping):
        raise ValueError(f"map generation profile must be a JSON object: {path}")
    source = raw.get("map_generation", raw)
    data = source.get("content_total_ranges")
    if data is None:
        data = source.get("content_totals", source)
    if not isinstance(data, Mapping):
        raise ValueError(f"missing floor content ranges in {path}")
    result: dict[int, Mapping[str, tuple[int, int]]] = {}
    for raw_floor, category_payload in data.items():
        floor = _floor_key_to_int(raw_floor)
        if floor is None:
            raise ValueError(f"invalid floor key in {path}: {raw_floor!r}")
        if not isinstance(category_payload, Mapping):
            raise ValueError(f"invalid floor payload in {path}: {category_payload!r}")
        result[floor] = _normalise_category_ranges(category_payload)
    if set(result) - {1, 2, 3, 4, 5, 6}:
        raise ValueError(f"invalid floor in {path}: {set(result)}")
    return result


def _parse_resource_delta(text: str) -> ResourceDelta:
    if not text:
    ...
    for part in parts:
        if not part:
            continue
        negative = any(token in part for token in ("失去", "减少", "扣", "支付", "消耗", "花费"))
        sign = -1 if negative else 1
        for raw_amount, raw_resource in re.findall(
            r"(\d+)\s*(源石锭|锭|希望|生命上限|生命|护盾|零件|收藏品|行动力|行动点|招募券|战斗票|追猎票|追捕票|战术牌|作战票)",
            part,
        ):
            amount = sign * int(raw_amount)
            match raw_resource:
                case "源石锭" | "锭":
                    delta = ResourceDelta(
                        gold=delta.gold + amount,
                        max_hp=delta.max_hp,
                        hp=delta.hp,
                        shield=delta.shield,
                        relics=delta.relics,
                        ...
                        team_strength=delta.team_strength,
                        action_points=delta.action_points,
                    )
                case "希望":
                    delta = ResourceDelta(
                        hope=delta.hope + amount,
                        gold=delta.gold,
                        max_hp=delta.max_hp,
                        hp=delta.hp,
                        shield=delta.shield,
                ...
                case "行动力" | "行动点":
                    delta = ResourceDelta(
                        action_points=delta.action_points + amount,
                        gold=delta.gold,
                        hp=delta.hp,
                        max_hp=delta.max_hp,
                        shield=delta.shield,
                ...
                case "招募券" | "作战票" | "追猎票" | "追捕票":
                    delta = ResourceDelta(
                        tickets=delta.tickets + amount,
                        gold=delta.gold,
                        hp=delta.hp,
                        max_hp=delta.max_hp,
                        shield=delta.shield,
                ...
                case "战术牌" | "战斗票":
                    delta = ResourceDelta(
                        team_strength=delta.team_strength + amount,
                        gold=delta.gold,
                        hp=delta.hp,
                        max_hp=delta.max_hp,
                        shield=delta.shield,
...
def _parse_options(payload: Any) -> tuple[EventOption, ...]:
    ...
    for index, item in enumerate(payload):
        ...
        option_id = str(
            item.get("option_id")
            or item.get("id")
            or re.sub(r"\W+", "_", title).strip("_")
            or f"option_{index}"
        )
        delta = ResourceDelta()
        if "delta" in item and isinstance(item.get("delta"), Mapping):
            delta_payload = item["delta"]
            delta = ResourceDelta(
                gold=int(delta_payload.get("gold", 0)),
                hope=int(delta_payload.get("hope", 0)),
                hp=int(delta_payload.get("hp", 0)),
                max_hp=int(delta_payload.get("max_hp", 0)),
                shield=int(delta_payload.get("shield", 0)),
                parts=int(delta_payload.get("parts", 0)),
                relics=int(delta_payload.get("relics", 0)),
                tickets=int(delta_payload.get("tickets", 0)),
                team_strength=int(delta_payload.get("team_strength", 0)),
                action_points=int(delta_payload.get("action_points", 0)),
            )
        else:
            delta = _parse_resource_delta(str(item.get("result", "")))
            if "delta_hp" in item:
                delta = ResourceDelta(
                    hp=int(item["delta_hp"]),
                    max_hp=delta.max_hp,
                    gold=delta.gold,
                    shield=delta.shield,
                    hope=delta.hope,
        ...
        add_items = tuple(
            map(str, item.get("add_items", item.get("add", ())))
            if isinstance(item.get("add_items", item.get("add", ())), (list, tuple))
            else (str(item.get("add") ),)
            if item.get("add") is not None
            else ()
        )
        remove_items = tuple(
            map(str, item.get("remove_items", item.get("remove", ())))
            if isinstance(item.get("remove_items", item.get("remove", ())), (list, tuple))
            else (str(item.get("remove") ),)
            if item.get("remove") is not None
            else ()
        )
        battle = bool(item.get("battle") or item.get("combat") or False)
        options.append(
            EventOption(
                option_id=option_id,
                title=title,
                effect=delta,
                add_items=tuple(dict.fromkeys(add_items)),
                remove_items=tuple(dict.fromkeys(remove_items)),
                battle=battle,
            )
        )
...
def load_node_reward_profile(
    path: str | Path | None,
) -> NodeRewardProfile | None:
    if path is None:
        return None
    raw = json.loads(Path(path).read_text(encoding="utf-8"))
    payload = raw
    if not isinstance(payload, Mapping):
        raise ValueError(f"node reward profile must be a JSON object: {path}")
    if isinstance(payload.get("node_rewards"), Mapping):
        source = payload["node_rewards"]
    else:
        source = payload

    incident_pools: dict[int, tuple[str, ...]] = {}
    node_overrides: dict[NodeType, RewardOverride] = {}
    incident_overrides: dict[str, tuple[EventOption, ...]] = {}

    candidate_pools = payload.get("incident_event_pools", payload.get("event_pools"))
    if isinstance(candidate_pools, Mapping):
        for raw_floor, events in candidate_pools.items():
            floor = _floor_key_to_int(raw_floor)
            if floor is None:
                raise ValueError(f"invalid floor key in incident pool: {raw_floor!r}")
            if not isinstance(events, list):
                raise ValueError(f"invalid event pool for floor {floor}")
            incident_pools[floor] = tuple(str(item) for item in events)

    if "incidents" in payload and isinstance(payload["incidents"], Mapping):
        for event_name, item in payload["incidents"].items():
            event_options = _parse_options(item.get("options") if isinstance(item, Mapping) else None)
            if event_options:
                incident_overrides[str(event_name)] = event_options
    ...
    if "events" in payload and isinstance(payload["events"], list):
        for item in payload["events"]:
            ...
            if not name or options is None:
                continue
            if str(node_type).upper() == "INCIDENT":
                incident_overrides[str(name)] = options

    if isinstance(payload.get("node_rewards"), Mapping):
        source = payload["node_rewards"]
    elif isinstance(payload.get("nodes"), Mapping):
        source = payload["nodes"]
    for raw_type, item in source.items():
        if not isinstance(item, Mapping):
            continue
        if isinstance(item.get("node_type"), str):
            raw_type = item.get("node_type")
        try:
            node_type = NodeType(raw_type)
        except ValueError:
            continue
        if "event_name" not in item and not any(
            key in item for key in ("options", "auto_effect", "reward")
        ):
            continue
        event_name = str(item.get("event_name") or item.get("label") or node_type.value)
        event_options = _parse_options(item.get("options"))
        if not event_options and isinstance(item.get("options"), list):
            # explicit empty list means explicit no-choice node content
            event_options = ()
        auto_payload = item.get("auto_effect", item.get("auto_reward", item.get("reward", {})))
        if isinstance(auto_payload, Mapping):
            auto_effect = ResourceDelta(
                gold=int(auto_payload.get("gold", 0)),
                hp=int(auto_payload.get("hp", 0)),
                max_hp=int(auto_payload.get("max_hp", 0)),
                shield=int(auto_payload.get("shield", 0)),
                hope=int(auto_payload.get("hope", 0)),
                parts=int(auto_payload.get("parts", 0)),
                relics=int(auto_payload.get("relics", 0)),
                tickets=int(auto_payload.get("tickets", 0)),
                team_strength=int(auto_payload.get("team_strength", 0)),
                action_points=int(auto_payload.get("action_points", 0)),
            )
        elif isinstance(auto_payload, str):
            auto_effect = _parse_resource_delta(auto_payload)
        else:
            auto_effect = ResourceDelta()
        node_overrides[node_type] = RewardOverride(
            event_name=event_name,
            options=event_options,
            auto_effect=auto_effect,
        )

    source_text = json.dumps(raw, sort_keys=True, ensure_ascii=False)
    return NodeRewardProfile(
        incident_pools=dict(incident_pools),
        node_overrides=dict(node_overrides),
        incident_overrides=incident_overrides,
        source_digest=sha256(source_text.encode("utf-8")).hexdigest(),
    )


def _content_total_fingerprint(
    overrides: Mapping[int, Mapping[str, tuple[int, int]]]
) -> str:
    payload = {}
    for floor, category_payload in sorted(overrides.items()):
        payload[floor] = {
            "mystery": tuple(category_payload.get("mystery", (0, 0))),
            "ferocity": tuple(category_payload.get("ferocity", (0, 0))),
        }
    return sha256(
...
def _normalise_content_total_ranges(
    overrides: Mapping[int, Mapping[str, tuple[int, int]]] | None
) -> dict[int, dict[str, tuple[int, int]]]:
    result: dict[int, dict[str, tuple[int, int]]] = {}
    for floor, payload in _CONTENT_TOTAL_RANGES.items():
        result[floor] = {
            "mystery": tuple(payload["mystery"]),
            "ferocity": tuple(payload["ferocity"]),
        }
    if overrides is not None:
        for floor, payload in overrides.items():
            merged = _normalise_category_ranges(payload)
            result[floor] = {
                "mystery": merged["mystery"],
                "ferocity": merged["ferocity"],
            }
    return result
...
class MapGenerator:
    ...
    def __init__(
        ...
        map_generation_profile: Mapping[int, Mapping[str, tuple[int, int]]] | None = None,
        node_reward_profile: NodeRewardProfile | None = None,
    ) -> None:
        ...
        self.node_reward_profile = node_reward_profile
        self.content_total_ranges = _normalise_content_total_ranges(map_generation_profile)
        self.map_generation_profile_digest = (
            _content_total_fingerprint(self.content_total_ranges)
            if map_generation_profile is not None
            else "default"
        )
        self.node_reward_profile_digest = (
            node_reward_profile.source_digest
            if node_reward_profile is not None
            else "default"
        )
        self._incident_pools: dict[int, tuple[str, ...]] = {
            floor: tuple(names) for floor, names in self.ruleset.event_pools.items()
        }
        if node_reward_profile is not None:
            self._incident_pools.update(node_reward_profile.incident_pools)
    ...
    def _assign_content(
        ...
        *,
        ...
        total_ranges = self.content_total_ranges[floor]
    ...
    def _incident_event_pool(self, floor: int) -> tuple[str, ...]:
        return self._incident_pools.get(floor, self.ruleset.event_pools[floor])
    ...
    def _content_for_node(
        ...
        if self.node_reward_profile is not None:
            reward_override = self.node_reward_profile.node_overrides.get(node_type)
            if reward_override is not None:
                return (
                    reward_override.event_name,
                    reward_override.options,
                    reward_override.auto_effect,
                    False,
                )
        if not self.config.allow_synthetic_event_effects:
            ...
            if node_type is NodeType.INCIDENT:
                event_name = rng.choice(self._incident_event_pool(floor))
                incident_override = (
                    self.node_reward_profile.incident_overrides.get(event_name)
                    if self.node_reward_profile is not None
                    else None
                )
                if incident_override is not None:
                    return event_name, incident_override, ResourceDelta(), False
    ...
    def _incident_options(
        ...
        if self.node_reward_profile is not None:
            incident_override = self.node_reward_profile.incident_overrides.get(event_name)
            if incident_override is not None:
    ...
    def validate(self, floor_map: FloorMap) -> None:
        ...
        for category, (minimum, maximum) in self.content_total_ranges[
            floor_map.floor
        ].items():
