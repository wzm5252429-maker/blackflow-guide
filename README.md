...
`modules/nonbattle-control/` 提供独立、冻结依赖的 Windows 本机接管器：任意宽高比截图、MAA 视觉观测、当前真实神经网络、逐帧点击闭环以及网站「自动路线决策」入口。开始战斗和战斗输入排除在接管之外；默认仅启动本机服务，用户点击网站按钮后才启用游戏输入。

使用说明和已知覆盖边界见 [非战斗接管](modules/nonbattle-control/README.md)。已通过截图回放与模拟输入测试，尚未达到所有非战斗页面直到一结局的完整实机验收。冻结模块保留当前研究核心及权重，不覆盖既有训练目录。
